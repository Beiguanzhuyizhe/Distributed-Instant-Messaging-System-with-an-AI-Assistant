...
客户端 `client/connection.py` 具备断线检测与指数退避重连逻辑，最多尝试 5 次。CLI 中断线时会显示：
...
但当前实现重连的是 TCP 连接本身，重连后不会自动补发登录请求并恢复认证态。因此，现场稳定演示建议展示“断线提示、服务端重启、客户端重连提示”，随后手动重新登录或重启客户端恢复通信。若要把它作为完整的自动重连演示，应补充“重连后自动重新登录并恢复会话”的代码。
...
2. **补充重连后的自动重新登录**
   ...
   客户端已能自动重连 TCP，但重连后未自动恢复登录态。建议在连接恢复回调中保存并重发登录凭据，成功后刷新在线用户与当前会话状态，这样更符合“服务器断线重连场景”的完整演示要求。
...
在隔离可写数据库路径下，项目核心功能通过端到端测试：注册、登录、私聊、群聊、在线用户、撤回、历史和心跳均通过。压力测试达到作业要求的 50 客户端并发和 100 用户同时在线，并进一步通过 150 客户端、100 客户端 1000 条消息、200 客户端连接上限测试。真实 DeepSeek AI smoke test 成功返回中文回答。主要风险集中在当前工作区 SQLite I/O 环境、重连后未自动恢复登录态，以及文件传输“断点续传”语义还不够完整。
